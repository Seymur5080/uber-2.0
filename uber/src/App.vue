...
<template>
  <div>
    ...
    <!-- --------------- SIGN IN --------------- -->
    <div class="sign-btn">
      <button class="general-btn" @click.prevent="status = true">
    ...
    <!-- --------------- SIGN BOX --------------- -->
    <div class="sign_box" v-show="status">
      <form action="POST" @submit.prevent="send">
        <div class="sign_box_exit_icon" @click.prevent="status = false">
          <i class="fa-solid fa-xmark"></i>
        </div>
        <h1 class="sign_box_title">
          {{ show == false ? "Войти" : "Регистрация" }}
        </h1>
...
export default {
  ...
  methods: {
    send() {
      this.axios
        .post(
          "https://identitytoolkit.googleapis.com/v1/accounts:signInWithCustomToken?key=AIzaSyAooYWJbHv2kKmNkO0OwFcbZ9qedtI72rU",
          {
...
.sign_box .sign_box_flex .sign_box_flex_left {
  display: flex;
  justify-content: space-between;
  align-items: center;
}

.sign_box .sign_box_flex .sign_box_flex_left input {
  width: 15px;
  height: 15px;
  border-radius: 50%;
}

.sign_box .sign_box_flex .sign_box_flex_left label {
  margin-left: 10px;
...
.sign_box .sign_box_flex .sign_box_flex_left label:hover {
  color: #808080;
}
